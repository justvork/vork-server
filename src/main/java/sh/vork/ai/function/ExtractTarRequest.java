...
public record ExtractTarRequest(
        @JsonProperty(required = true, value = "archivePath")
        @JsonPropertyDescription("Path to the source tar file in the selected area.")
        String archivePath,

        @JsonProperty(required = true, value = "destinationPath")
        @JsonPropertyDescription("Destination folder path where tar contents are extracted.")
